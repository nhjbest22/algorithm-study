...
void solve() {
  ...
  rep(i, 1, N + 1) {
    ...
    rep(j, 0, i) {
      ...
      // 2. 이번 문제 맞춘다. -> streak + 1
      next_dp[j + 1] = dp[j] << score;
    }

    rep(j, 0, i + 1) swap(next_dp[j], dp[j]);
  }
}
...
int main() {
  ...
  bitset<MAX + 10> result;
  rep(i, 0, N + 1) result |= dp[i];
