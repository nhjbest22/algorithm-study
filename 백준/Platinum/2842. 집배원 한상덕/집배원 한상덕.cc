...
bool check(int minH, int maxH) {
  if (arr[st.X][st.Y] < minH || arr[st.X][st.Y] > maxH) return false;

  int cnt = 0;

  fill(&visit[0][0], &visit[0][0] + 55 * 55, false);
  queue<pair<int, int>> Q;
  ...
  while (!Q.empty()) {
    ...
    rep(dir, 0, 8) {
      ...
      if (arr[x][y] < minH || arr[x][y] > maxH) continue;
...
int main() {
  ...
  rep(i, 0, N) {
    rep(j, 0, N) {
      if (board[i][j] == 'P') st = {i, j};
      if (board[i][j] == 'K') house++;
    }
  }
