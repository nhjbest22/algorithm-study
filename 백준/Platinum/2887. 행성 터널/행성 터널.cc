...
int main(){
  ...
  rep(i,0,N-1){
    edge[e++] = make_tuple(x[i+1].first - x[i].first, x[i+1].second, x[i].second);
    edge[e++] = make_tuple(y[i+1].first - y[i].first, y[i+1].second, y[i].second);
    edge[e++] = make_tuple(z[i+1].first - z[i].first, z[i+1].second, z[i].second);
  }
  sort(edge, edge+e);
  rep(i,0,e){
    ll cost; int a,b;
