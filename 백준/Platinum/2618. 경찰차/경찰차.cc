...
int dfs(int a, int b) {
  if (max(a, b) == W + 1) return 0;
  if (dp[a][b] > -1) return dp[a][b];

  dp[a][b] = 0;
  int nxtCase = max(a, b) + 1;
  int f = dfs(nxtCase, b) + dist(nxtCase, a);
  int s = dfs(a, nxtCase) + dist(nxtCase, b);

  visit[a][b] = f < s ? 0 : 1;

  return dp[a][b] = min(f, s);
}

void find_route(int a, int b) {
  while (max(a, b) != W + 1) {
    int nxtCase = max(a, b) + 1;
    cout << visit[a][b] + 1 << endl;
    visit[a][b] ? b = nxtCase : a = nxtCase;
  }
}
