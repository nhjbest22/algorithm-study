...
int main() {
  ...
  while (!pq.empty()) {
    ...
    for (auto& [nxt_cost, nxt_idx] : adj[cur_idx]) {
      if (cost[nxt_idx].size() == K) {
        if (cost[nxt_idx].top() <= cur_cost + nxt_cost) continue;

        cost[nxt_idx].pop();
      }

      cost[nxt_idx].push(cur_cost + nxt_cost);
