...
bool check(int mid) {
  rep(i, 0, 201) {
    if (arr[0][0] > i + mid || arr[0][0] < i) continue;
    if (arr[N - 1][N - 1] > i + mid || arr[N - 1][N - 1] < i) continue;

    Q = queue<pair<int, int>>();
    fill(&visit[0][0], &visit[0][0] + 105 * 105, false);
    ...
    while (!Q.empty()) {
      ...
      rep(dir, 0, 4) {
        ...
        if (arr[x][y] < i || arr[x][y] > i + mid) continue;
