...
int main(){
  ...
  Q.push({1,1}); vis[1][1] = true; on[1][1] = true;
  while(!Q.empty()){
    ...
    for(auto nxt: adj[cur.first][cur.second]){
      ...
      bool isAdj = false;
      rep(i,0,4){
        int x = nxt.first + dx[i];
        int y = nxt.second + dy[i];
        if(x <= 0 || x > N || y <= 0 || y > N) continue;
        if(vis[x][y]) isAdj = true;
      }
      if(isAdj){
        Q.push({nxt.first, nxt.second}); vis[nxt.first][nxt.second] = true;
      }
    }
  }
  rep(i,1,N+1){
