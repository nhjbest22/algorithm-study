...
bool postable(int x, int y) {
  rep(i, 0, R) {
    rep(j, 0, C) {
      if (paper[i][j] && note[x + i][y + j]) return false;
    }
  }

  rep(i, 0, R) {
    rep(j, 0, C) { note[x + i][y + j] += paper[i][j]; }
  }
...
void rotate() {
  bool tmp[12][12];
  rep(i, 0, R) { rep(j, 0, C) tmp[i][j] = paper[i][j]; }

  rep(i, 0, C) {
    rep(j, 0, R) { paper[i][j] = tmp[R - 1 - j][i]; }
  }
