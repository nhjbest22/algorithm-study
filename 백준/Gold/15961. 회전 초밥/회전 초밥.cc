...
int sushi[3'003'005];
int cnt[3'003'005];
int D, K, C;  // 초밥의 가짓수, 연속해서 먹는 접시의 수, 쿠폰 번호
int cur, ans;
...
int main() {
  ...
  int N;
  cin >> N >> D >> K >> C;
  rep(i, 0, N) cin >> sushi[i + K - 1];
  // 0 ~ K - 2 까지는 마지막 스시 재입력
  // 전체 스시 개수 N + K -1
  // 0 ~ N - K - 2

  // K - 2 -> N + K - 2
  rep(i, 0, K - 1) sushi[i] = sushi[i + N];

  rep(i, 0, K - 1) {
    cnt[sushi[i]]++;
    if (cnt[sushi[i]] == 1) cur++;
  }

  int st = 0;
  for (int en = K - 1; en < N + K - 1; en++) {
    cnt[sushi[en]]++;
    if (cnt[sushi[en]] == 1) cur++;

    ans = max(ans, cur + (cnt[C] == 0));

    cnt[sushi[st]]--;
    if (cnt[sushi[st]] == 0) cur--;
    st++;
  }
