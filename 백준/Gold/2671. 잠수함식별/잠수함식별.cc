...
bool dp[155];
// dp[i] -> i번째 까지의 string이 패턴과 매치되는지
...
int main() {
  ...
  rep(i, 0, inp.size()) {
    ...
    if (ch == '0') {
      if (idx < inp.size() && inp[idx] == '1') dp[i + 2] = true;

      continue;
    } else {
      int zeroCnt = 0;
      while (idx < inp.size() && inp[idx] == '0') {
        zeroCnt++;
        idx++;
      }
  ...
  cout << (dp[inp.size()] ? "SUBMARINE" : "NOISE");
