...
// idx -> x, y, dist
int x[50'005], y[50'005], dist[50'005];
vector<pair<int, int>> v[200'005];  // y -> (x, idx)
int N, T;
...
int main() {
  ...
  rep(i, 0, T + 1) sort(v[i].begin(), v[i].end());
  ...
  while (!Q.empty()) {
    ...
    rep(ny, y[cur] - 2, y[cur] + 3) {
      if (ny < 0 || ny > T) continue;
      auto lo =
          lower_bound(v[ny].begin(), v[ny].end(), pii{x[cur] - 2, INT32_MIN});
      auto hi =
          upper_bound(v[ny].begin(), v[ny].end(), pii{x[cur] + 2, INT32_MAX});

      for (auto nxt = lo; nxt < hi; nxt++) {
        auto [nx, idx] = *nxt;
