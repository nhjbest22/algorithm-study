...
vector<int> route(105);

int main() {
  ios::sync_with_stdio(0);
  cin.tie(0);

  fill(&cost[0][0], &cost[0][0] + 105 * 105, INF);
  ...
  rep(i, 1, N + 1) cost[i][i] = 0;

  rep(mid, 1, N + 1) {
    rep(u, 1, N + 1) {
      rep(v, 1, N + 1) {
        if (cost[u][v] > cost[u][mid] + cost[mid][v]) {
          cost[u][v] = cost[u][mid] + cost[mid][v];
          nxt[u][v] = nxt[u][mid];
        }
      }
    }
