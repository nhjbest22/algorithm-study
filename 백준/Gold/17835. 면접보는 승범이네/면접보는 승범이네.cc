...
int main() {
  ...
  fill(dist, dist + 100'005, INT64_MAX);

  rep(i, 0, K) {
    int loc;
    cin >> loc;
    adj[0].push_back({0, loc});
  }
  ...
  while (!pq.empty()) {
    ...
    for (auto& [nxt_cost, nxt] : adj[cur]) {
      if (dist[nxt] <= nxt_cost + cur_cost) continue;

      dist[nxt] = nxt_cost + cur_cost;
      pq.push({dist[nxt], nxt});
    }
  }

  auto it = max_element(dist, dist + N + 1);
  cout << it - dist << endl;
  cout << *it;
