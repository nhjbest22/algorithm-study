...
vector<pair<int, int>> adj_rev[1'005];

int dist[1'005], dist_rev[1'005];
...
int main() {
  ...
  while (!pq.empty()) {
    ...
    if (dist_rev[cur] != cur_cost) continue;

    for (auto& [nxt_cost, nxt] : adj_rev[cur]) {
      if (dist_rev[nxt] <= cur_cost + nxt_cost) continue;

      dist_rev[nxt] = cur_cost + nxt_cost;
      pq.push({dist_rev[nxt], nxt});
    }
  }

  int MAX = 0;
  rep(i, 1, N + 1) MAX = max(MAX, dist[i] + dist_rev[i]);
