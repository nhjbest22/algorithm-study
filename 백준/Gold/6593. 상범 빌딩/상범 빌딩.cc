...
int main()
{
  ...
  while (1)
  {
    ...
    string board[35][35];
    queue<tuple<int, int, int>> Q; // z, x, y 순서 (L, R, C)
    fill(&dist[0][0][0], &dist[0][0][0] + 35 * 35 * 35, -1);
    ...
    while (!Q.empty())
    {
      ...
      {
        cout << "Escaped in " << dist[curZ][curX][curY] << " minute(s)." << endl;
        isEscaped = true;
        break;
      }
