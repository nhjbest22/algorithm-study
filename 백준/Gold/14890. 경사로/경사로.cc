...
bool check(vector<int> &line) {
  int idx(0), cnt(1);  // 보고 있는 칸의 index, 같은 길이의 칸 수

  while (idx < N - 1) {
    if (abs(line[idx + 1] - line[idx]) > 1) return false;
...
int main() {
  ...
  rep(i, 0, N) {
    vector<int> line;
    rep(j, 0, N) line.push_back(board[i][j]);
    if (check(line)) ans++;
  }

  rep(i, 0, N) {
    vector<int> line;
    rep(j, 0, N) { line.push_back(board[j][i]); }
    if (check(line)) ans++;
  }
