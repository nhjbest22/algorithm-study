...
int main() {
  ...
  rep(i, 0, N) {
    rep(j, 0, M) {
      ...
      ability[i].push_back(num);
    }

    sort(ability[i].begin(), ability[i].end());
    rep(j, 0, M) { v.push_back({ability[i][j], i, j}); }
  }

  sort(v.begin(), v.end());

  rep(i, 0, N) {
    MIN = min(MIN, ability[i][0]);
    MAX = max(MAX, ability[i][0]);
  }
