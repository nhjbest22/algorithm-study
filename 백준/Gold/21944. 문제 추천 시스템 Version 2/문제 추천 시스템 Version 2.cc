...
pair<int, int> P2GL[100'005];

set<tuple<int, int, int>> GLP;
set<pair<int, int>> LP;
...
int main() {
  ...
  rep(i, 0, M) {
    ...
    if (query == "recommend") {
      int G, x;
      cin >> G >> x;
      auto it = (x > 0 ? --GLP.lower_bound({G + 1, 0, 0})
                       : GLP.lower_bound({G, 0, 0}));
      ...
      int x;
      cin >> x;
      auto it = (x > 0 ? --LP.end() : LP.begin());
      cout << it->Y << endl;

    } else if (query == "recommend3") {
      int x, L;
      cin >> x >> L;
      auto it = LP.lower_bound({L, 0});

      if (x > 0)
        cout << (it == LP.end() ? -1 : it->Y) << endl;
      else
        cout << (it == LP.begin() ? -1 : (--it)->Y) << endl;
