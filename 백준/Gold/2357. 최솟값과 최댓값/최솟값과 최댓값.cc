...
int max_tree[200'005];
int min_tree[200'005];
...
void query(int l, int r) {
  ...
  while (l <= r) {
    if (l % 2 == 1) {
      MAX = max(MAX, max_tree[l]);
      MIN = min(MIN, min_tree[l]);
      l++;
    }
    if (r % 2 == 0) {
      MAX = max(MAX, max_tree[r]);
      MIN = min(MIN, min_tree[r]);
      r--;
    }
