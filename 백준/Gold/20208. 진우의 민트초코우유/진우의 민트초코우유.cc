...
int main() {
  ...
  rep(i, 0, N) {
    rep(j, 0, N) {
      cin >> board[i][j];
      if (board[i][j] == 1) home = {i, j};
      if (board[i][j] == 2) milks.push_back({i, j});
    }
  }

  int seq[10];
  rep(i, 0, milks.size()) seq[i] = i;

  do {
    auto cur = home;
    int health = M;
    rep(i, 0, milks.size()) {
      auto nxt = milks[seq[i]];
      int dist = abs(nxt.X - cur.X) + abs(nxt.Y - cur.Y);

      if (health < dist) break;
      ...
      dist = abs(cur.X - home.X) + abs(cur.Y - home.Y);

      if (health >= dist) ans = max(ans, i + 1);
    }
  } while (next_permutation(seq, seq + milks.size()));
