...
void inOrder(int cur, int level) {
  ...
  left_most[level] = min(left_most[level], counter);
  right_most[level] = max(right_most[level], counter);
...
int main() {
  ...
  int N;
  cin >> N;
  fill(left_most, left_most + MAX, MAX);
  ...
  int level = 0;
  rep(i, 1, N + 1) {
    int W = right_most[i] - left_most[i] + 1;
