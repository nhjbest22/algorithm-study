...
int main() {
  ...
  rep(i, 0, N) {
    int num;
    cin >> num;
    arr[i] = arr_sort[i] = num;
  }
  sort(arr_sort, arr_sort + N);

  int idx = 1;
  rep(i, 1, N) if (arr_sort[i - 1] != arr_sort[i]) arr_sort[idx++] =
      arr_sort[i];

  rep(i, 0, N) cout << lower_bound(arr_sort, arr_sort + idx, arr[i]) - arr_sort
                    << ' ';
