...
int main() {
  ...
  while (T--) {
    fill(isContained, isContained + 105, true);
    vector<string> words;
    ...
    int pos = 0;
    while (pos < inp.size()) {
      int npos = inp.find(' ', pos);

      // abc def gh
      // 0123456789
      // npos -> 3, 7
      if (npos == -1) npos = inp.size();
      if (npos - pos > 0) words.push_back(inp.substr(pos, npos - pos));
    ...
    while (1) {
      ...
      // int pos = str.find("goes");
      int pos = str.find(' ');
      pos = str.find(' ', pos + 1);
      string bark = str.substr(pos + 1);

      rep(i, 0, words.size()) {
        if (words[i] == bark) isContained[i] = false;
      }
    }

    rep(i, 0, words.size()) {
      if (isContained[i]) cout << words[i] << ' ';
    }
