...
int inp[100005][3];
int dp[100005][3];
int maxCost = 200000000;

void solve() {
  dp[0][0] = maxCost;
  dp[0][1] = inp[0][1];
  dp[0][2] = dp[0][1] + inp[0][2];
  ...
  rep(i, 1, N) {
    rep(j, 0, 3) {
      rep(dir, 0, 4) {
        ...
        // dp[i][j] = dp[i][j] ? min(dp[i][j], dp[x][y] + inp[i][j])
        //                     : dp[x][y] + inp[i][j];

        dp[i][j] = min(dp[i][j], dp[x][y] + inp[i][j]);
      }
    }
  }
}
...
int main() {
  ...
  while (1) {
    ...
    rep(i, 0, N) { rep(j, 0, 3) cin >> inp[i][j]; }
    fill(&dp[0][0], &dp[0][0] + 3 * 100005, maxCost);
    solve();
